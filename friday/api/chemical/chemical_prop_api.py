...
class ChemicalPropAPI:
    ...
    def get_name_by_cid(self, cid: str, top_k: Optional[int] = None) -> List[str]:
        html_doc = safe_requests.get(f"{self._endpoint}cid/{cid}/synonyms/XML").text
    ...
    def get_cid_by_struct(self, smiles: str) -> List[str]:
        html_doc = safe_requests.get(f"{self._endpoint}smiles/{smiles}/cids/XML").text
        soup = BeautifulSoup(html_doc, "html.parser", from_encoding="utf-8")
    ...
    def get_cid_by_name(self, name: str, name_type: Optional[str] = None) -> List[str]:
        ...
        html_doc = safe_requests.get(url).text
        soup = BeautifulSoup(html_doc, "html.parser", from_encoding="utf-8")
        cids = soup.find_all('cid')
        if cids is None:
            return []
    ...
    def get_prop_by_cid(self, cid: str) -> str:
        html_doc = safe_requests.get(f"{self._endpoint}cid/{cid}/property/MolecularFormula,MolecularWeight,CanonicalSMILES,IsomericSMILES,IUPACName,XLogP,ExactMass,MonoisotopicMass,TPSA,Complexity,Charge,HBondDonorCount,HBondAcceptorCount,RotatableBondCount,HeavyAtomCount,CovalentUnitCount/json").text
